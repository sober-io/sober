...
pub async fn run_turn<R: AgentRepos>(params: &TurnParams<'_, R>) -> Result<(), AgentError> {
    ...
    for iteration in 0..params.config.max_tool_iterations {
        ...
        // Reasoning content buffer — currently unused because MessageDelta
        // doesn't expose reasoning_content. When sober-llm adds streaming
        // reasoning support, populate this from the stream chunks.
        let reasoning_buffer = String::new();
        // tool call index → (id, name, arguments_buffer)
        ...
        while let Some(chunk_result) = stream.next().await {
            ...
            // Capture usage from any chunk (typically the final one).
        ...
        // Assemble final tool calls from the buffered deltas.
        let tool_calls: Vec<LlmToolCall> = if tool_call_buffers.is_empty() {
            Vec::new()
        } else {
            let mut entries: Vec<(u32, (String, String, String))> =
                tool_call_buffers.into_iter().collect();
