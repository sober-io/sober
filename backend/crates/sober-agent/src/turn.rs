...
pub async fn run_turn<R: AgentRepos>(params: &TurnParams<'_, R>) -> Result<(), AgentError> {
    ...
    for iteration in 0..params.ctx.config.max_tool_iterations {
        ...
        if has_tool_calls {
            ...
            // Repetitive tool call circuit breaker: if the LLM calls the exact
            // same tool(s) with the exact same arguments as last iteration,
            // hard-stop — the result won't change on a repeat call.
            let signature = {
            ...
            if last_tool_signature.as_deref() == Some(&signature) {
                warn!(signature, "repetitive tool call detected, stopping turn");
                break;
            }
            ...
            event: Some(proto::conversation_update::Event::NewMessage(
                proto::NewMessage {
                    ...
                    source: match params.trigger {
                        ...
                        TriggerKind::Scheduler => proto::MessageSource::Scheduler,
                        TriggerKind::Replica => proto::MessageSource::Replica,
                        TriggerKind::Admin => proto::MessageSource::Admin,
                    }
                    .into(),
