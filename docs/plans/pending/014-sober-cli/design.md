...
- Creates a pool via `sober_db::create_pool()` with `max_connections=1` (CLI does not need concurrency).
- Constructs repo implementations from `sober-db` (e.g., `PgUserRepo`, `PgSessionRepo`) for database operations.
...
| Crate | Purpose |
|-------|---------|
| sober-core | Types, config, errors, repo traits |
| sober-crypto | Password hashing (Argon2id) --- new dependency, update architecture docs |
| sober-db | Pool creation (`create_pool()`), repo implementations (PgUserRepo, etc.) |
