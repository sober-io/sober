...
2. Create module structure: `src/lib.rs`, `src/error.rs`, `src/password.rs`, `src/keys.rs`, `src/injection.rs`.
...
6. Implement `injection.rs`: `InjectionReport` struct, `check_injection` function with basic pattern list.
7. Write unit tests:
   ...
   - Injection: known patterns detected, clean input passes, score ranges.
8. Write property-based tests (proptest):
...
9. Run `cargo clippy -- -D warnings` and `cargo test -p sober-crypto`.
