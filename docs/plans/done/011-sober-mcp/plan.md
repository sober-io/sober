...
**Design doc:** `docs/plans/pending/011-sober-mcp/design.md`
...
- `sober-sandbox` (009) must be implemented. **Important:** the current sandbox design only has `BwrapSandbox::execute` (run-to-completion). MCP needs a `BwrapSandbox::spawn` method that returns a `Child` with captured stdin/stdout for long-running processes. This must be added to sober-sandbox before starting Task 5.
